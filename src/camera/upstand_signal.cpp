 ...
 *  全局变量定义
 */
int32_t  gl_gyro_ad         = 0;
int32_t  gl_acc_ad          = 0;
int32_t  gl_acc             = 0;    /* 用于显示在液晶上的加速度计的实时值 */
int32_t  gl_gyr             = 0;
int32_t  gl_angle_acc 	   = 0;
int32_t  gl_angle_gyro      = 0;
int32_t  gl_angle           = 0;
int32_t  gl_gyro_zero       = 327;
int32_t  gl_angle_real      = 0;
int32_t  gl_angle_gyro_real = 0;
int32_t  gl_acc_fix         = 0;

float printvars[3] = {0,0,0};
/**
...
//	  volatile float GyroscopeReal = 0;
	  ...
	  /* 传感器取值 */
	  gl_gyro_ad = omega[0];
	  gl_acc_ad = adc_once(RXADC, ADC_10bit) >> 1;

	  /* 设定零点 */
	  Accelerometer = (float)(ANGLE_ZERO ) - (float)gl_acc_ad;
//     Gyroscope     = ((float)gl_gyro_zero - (float)gl_gyro_ad + ((float)gl_speed_fb_fix - (float)gl_speed_lr_fix)/ 10.0);      /* 加速度计角度转化 */
      AngleAcc  = ACCY_AD2DEG_RATIO * Accelerometer;
      ...
      gl_angle_acc        = (int32_t)( AngleAcc * 100 );
...
//      gl_speed_angle      = (int32_t)( Posterior_Estimation * 10 - Posterior_Estimation_Real * 10 );
      ...
      /* 控制角度输出 */
      gl_angle            = (int32_t)( Posterior_Estimation * 10 );
      gl_angle_gyro       = (int32_t)( Gyroscope );
